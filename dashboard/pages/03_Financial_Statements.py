    ...
    for col in display.columns:
        ...
        try:
            display[col] = display[col].apply(
                lambda x: format_currency(x) if isinstance(x, float) and abs(x) > 1e4
                else (format_percentage(x) if isinstance(x, float) and abs(x) <= 2 else (f"{x:.2f}" if isinstance(x, float) else x))
            )
        except Exception:
            pass
    return display
...
with tab1:
    ...
    if not income_df.empty:
        ...
        st.dataframe(fmt_df(income_df.drop(columns=["ticker"], errors="ignore")), use_container_width=True, hide_index=True)
        
        st.markdown("---")
        income_sorted = income_df.sort_values("fiscal_year")
        col1, col2 = st.columns(2)
        with col1:
            fig = financial_bar_chart(income_sorted["fiscal_year"].tolist(), income_sorted["revenue"].fillna(0).tolist(), "Revenue", report.ticker)
...
with tab2:
    ...
    if not balance_df.empty:
        ...
        st.dataframe(fmt_df(balance_df.drop(columns=["ticker"], errors="ignore")), use_container_width=True, hide_index=True)

        balance_sorted = balance_df.sort_values("fiscal_year")
        col1, col2 = st.columns(2)
        with col1:
            fig = financial_bar_chart(balance_sorted["fiscal_year"].tolist(), balance_sorted["total_assets"].fillna(0).tolist(), "Total Assets", report.ticker, color=COLORS["gold"])
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = financial_bar_chart(balance_sorted["fiscal_year"].tolist(), balance_sorted["total_debt"].fillna(0).tolist(), "Total Debt", report.ticker, color=COLORS["bear"])
            st.plotly_chart(fig, use_container_width=True)
    else:
...
with tab3:
    ...
    if not cf_df.empty:
        ...
        st.dataframe(fmt_df(cf_df.drop(columns=["ticker"], errors="ignore")), use_container_width=True, hide_index=True)

        cf_sorted = cf_df.sort_values("fiscal_year")
        col1, col2 = st.columns(2)
        with col1:
            fig = financial_bar_chart(cf_sorted["fiscal_year"].tolist(), cf_sorted["operating_cash_flow"].fillna(0).tolist(), "Operating Cash Flow", report.ticker, color=COLORS["primary"])
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = financial_bar_chart(cf_sorted["fiscal_year"].tolist(), cf_sorted["free_cash_flow"].fillna(0).tolist(), "Free Cash Flow", report.ticker, color=COLORS["bull"])
            st.plotly_chart(fig, use_container_width=True)
    else:
