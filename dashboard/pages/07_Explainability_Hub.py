...
if classification:
    ...
    with col_probs:
        ...
        fig.update_layout(
            ...
            height=280,
            showlegend=False,
            annotations=[dict(text="P(class)", x=0.5, y=0.5, showarrow=False,
    ...
    if classification.feature_importances:
        ...
        fi = pd.DataFrame(list(classification.feature_importances.items()), columns=["Signal", "Strength"])
        fi = fi.sort_values("Strength", ascending=True)
        
        fig = go.Figure(go.Bar(
            x=fi["Strength"], y=fi["Signal"], orientation="h",
