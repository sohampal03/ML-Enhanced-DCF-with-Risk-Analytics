...
if not report.is_complete:
    st.error("Analysis incomplete. Please re-run.")
    st.stop()
...
with k2:
    metric_card("Enterprise Value", format_currency(dcf.enterprise_value), icon="🏗️")
with k3:
    metric_card("WACC", format_percentage(dcf.wacc), icon="⚖️")
with k4:
    metric_card("Terminal Growth", format_percentage(dcf.terminal_growth_rate), icon="∞")
with k5:
    metric_card("Beta", f"{info.beta:.2f}" if info.beta else "N/A", icon="📐")
with k6:
    metric_card(
        "Shares Out.",
        f"{info.shares_outstanding / 1e9:.2f}B" if info.shares_outstanding else "N/A",
        icon="📋",
    )
...
with col1:
    ...
    traffic_light(
        "Valuation Safety",
        "green" if mos > 0.20 else "yellow" if mos > 0 else "red",
        f"{mos*100:.1f}% MOS",
    )
    ...
    traffic_light(
        "P/E Ratio",
        "green" if pe_ok else "yellow" if pe_ok is None else "red",
    ...
    traffic_light(
        "Debt Level",
        "green" if de and de < 1 else "yellow" if de and de < 2 else "red",
...
if info.description:
    ...
    st.markdown(
        f'<div style="color:#8b9cc8;font-size:14px;line-height:1.8;max-width:1000px;">{info.description[:600]}...</div>',
        unsafe_allow_html=True,
    )
