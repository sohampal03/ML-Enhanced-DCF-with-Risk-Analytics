...
from src.data.schemas.financial_schemas import (
    FinancialData, CompanyInfo, AnnualIncomeStatement,
    AnnualBalanceSheet, AnnualCashFlowStatement,
)
...
def make_sample_data(ticker: str = "TEST") -> FinancialData:
    ...
    income = [
        AnnualIncomeStatement(
            ticker=ticker, fiscal_year=2023,
            revenue=100e9, ebit=20e9, pretax_income=18e9,
            income_tax=3.78e9, net_income=14.22e9,
            depreciation_amortization=5e9, ebit_margin=0.20,
        ),
        AnnualIncomeStatement(
            ticker=ticker, fiscal_year=2022,
            revenue=90e9, ebit=18e9, pretax_income=16e9,
            income_tax=3.36e9, net_income=12.64e9,
            depreciation_amortization=4.5e9, ebit_margin=0.20,
        ),
        AnnualIncomeStatement(
            ticker=ticker, fiscal_year=2021,
            revenue=80e9, ebit=15e9, pretax_income=13e9,
            income_tax=2.73e9, net_income=10.27e9,
            depreciation_amortization=4e9, ebit_margin=0.19,
        ),
    ]
    balance = [
        AnnualBalanceSheet(
            ticker=ticker, fiscal_year=year,
            total_debt=30e9, cash_and_equivalents=10e9,
            total_assets=200e9, total_equity=100e9,
    ...
    cashflow = [
        AnnualCashFlowStatement(
            ticker=ticker, fiscal_year=2023,
            operating_cash_flow=22e9, capital_expenditures=-5e9,
            free_cash_flow=17e9, depreciation_amortization=5e9,
        ),
        AnnualCashFlowStatement(
            ticker=ticker, fiscal_year=2022,
            operating_cash_flow=19e9, capital_expenditures=-4.5e9,
            free_cash_flow=14.5e9, depreciation_amortization=4.5e9,
        ),
        AnnualCashFlowStatement(
            ticker=ticker, fiscal_year=2021,
            operating_cash_flow=16e9, capital_expenditures=-4e9,
            free_cash_flow=12e9, depreciation_amortization=4e9,
        ),
    ]
    return FinancialData(
        company_info=CompanyInfo(
            ticker=ticker, name="Test Co", current_price=150.0,
            market_cap=500e9, shares_outstanding=3.33e9,
        ),
