...
class TestWACCEngine:
    ...
    def test_wacc_within_reasonable_bounds(self):
        data = make_sample_data()
        result = WACCEngine().compute(data)
    ...
    def test_capm_formula_string(self):
        data = make_sample_data()
        result = WACCEngine().compute(data)
...
class TestDCFEngine:
    ...
    def test_sensitivity_matrix_built(self):
        data = make_sample_data()
    ...
    def test_equity_value_non_negative(self):
        data = make_sample_data()
        result = DCFEngine().compute(data)
        assert result.equity_value >= 0

    def test_enterprise_value_components(self):
        data = make_sample_data()
        result = DCFEngine().compute(data)
